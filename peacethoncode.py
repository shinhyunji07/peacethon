...
def load_data():
    ...
    return {"chat_messages": [], "sns_posts": []}
...
if st.session_state.page_step == "profile":
    ...
    with col2:
        ...
        # 기존 선택된 유저 역할 기본값 유지
        default_index = 0 if "남한" in st.session_state.user_role else 1
        role_input = st.radio(
            "소속을 선택해주세요", 
            ["🇰🇷 남한 청년", "🇰🇵 북한 청년"],
            index=default_index
...
# ==========================================
    ...
    with col2:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("⬅️ 뒤로가기 (프로필 수정)", use_container_width=True):
